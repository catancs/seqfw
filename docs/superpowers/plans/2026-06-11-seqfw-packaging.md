...
## Task 3: cross-platform binary release workflow

**Files:**
- Create: `.github/workflows/release.yml`

A self-contained, hand-rolled release workflow (no `cargo-dist` generator
dependency) that triggers on `v*` tags, builds `seqfw` for the target matrix
(`x86_64-unknown-linux-musl`, `aarch64-apple-darwin`, `x86_64-apple-darwin`),
and attaches the tarballs to the GitHub Release via `softprops/action-gh-release`.

- [ ] **Step 1:** Write `.github/workflows/release.yml` with a `binaries` matrix
job: checkout, `dtolnay/rust-toolchain@stable` with the matrix `target`, install
`musl-tools` on the musl target, `cargo build --release -p seqfw-cli --target …`,
`tar` the binary, and upload on tag. `permissions: contents: write`.

- [ ] **Step 2:** Validate the YAML parses.

> **Operator follow-up:** richer installers (a `curl | sh` script + a generated
> Homebrew tap formula) can be layered later with `cargo dist init` / `cargo dist
> generate`, which reads `[workspace.metadata.dist]`; that tooling is not run by
> this plan. The matrix workflow above already ships installable binaries.

- [ ] **Step 3: Commit** `git commit -m "release: cross-platform binary release workflow"`.
